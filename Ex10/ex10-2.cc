...
int main( /* [Complete Here!!] */ int argc,char *argv[]){ //main関数の引数（ProgC のときと全く同じ）
  ...
  /* [Complete Here!!] */ //読み込みファイルオープン
  ifstream infile(argv[1]);

  if(/*[Complete Here!!]*/!infile.is_open()){ //エラー（読み込みファイルオープン失敗）時、標準エラーストリームに出力
    cerr << "error: no such input file named '" << argv[1] << "'." << endl;
    return -2;
  }


  /* [Complete Here!!] */  //開いた迷路ファイルからnx, ny（先頭の数字２つ）を読み取る
  infile >> nx >> ny;
  // cout << nx << " " << ny << endl;

  //２次元配列の動的確保（まず、ny個分の１次元配列領域を確保、１次元目）
  Maze =  /* [Complete Here!!] */ new int*[ny];
  Solution =  /* [Complete Here!!] */ new int*[ny];

  for(int j=0; j<ny; j++){  //１次元配列のそれぞれに対してnx個分の領域を確保（２次元目）
    Maze[j] = /* [Complete Here!!] */ new int[nx];
    Solution[j] = /* [Complete Here!!] */ new int[nx];

    for(int i=0;i<nx;i++){//確保した領域へ、ファイルから読み込んだ値（迷路の情報）を Maze[][]に格納
      /* [Complete Here!!] */ //１個ずつ読み込んで代入する操作が、for文でメモリ領域を確保した１行分繰り返される
      infile >> Maze[j][i];
      // cout << Maze[j][i];
    }
  }
  ...
  if(check== false){//解無し
    ...
    /* [Complete Here!!] */
    outfile.open(argv[2]);
    if(/* [Complete Here!!] */!outfile.is_open()){
      //エラー（書き込みファイルオープン失敗）時、標準エラーストリームに出力
  ...
  /* [Complete Here!!] */
  outfile.close();
  //もう使わないので、書き込みファイルストリームはクローズする
  for(int j=0; j<ny; j++){
    /* [Complete Here!!] */   //確保した Maze と Solutionの配列を２次元目を解放する(int型の配列；横)
    delete []Maze[j];
    delete []Solution[j];
  ...
  /* [Complete Here!!] */          //確保した Maze と Solutionの配列を１次元目を解放する(ポインタ型の配列；縦)
  delete []Maze;
  delete []Solution;
