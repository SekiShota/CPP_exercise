...
int main(){
  ...
  for(int day=1; x!=NULL || y!=NULL || z!=NULL ; day++){ //全部が同時に死んでいない限り飼育を続ける
    ...
    if(x!=NULL) x->Feed(feeds[0]); // x.Feed(feeds[0]); ==> x->Feed(feeds[0]);
    else x = new Wolf(100,20,"X"); // Wolf; ==> Wolf(100,20,X);
    if(y!=NULL) y->Feed(feeds[1]); // y.Feed(feeds[1]); ==> y->Feed(feeds[1]);
    else y = new Wolf(65,30,"Y");
    if(z!=NULL) z->Feed(feeds[2]); // z.Feed(feeds[2]); ==> z->Feed(feeds[2]);
  ...
  delete []feeds; //飼育員引退（最後にエサ箱を破棄する） delete feeds; ==> delete []feeds;
  /****************************** !!バグありここまで!! ******************************/
